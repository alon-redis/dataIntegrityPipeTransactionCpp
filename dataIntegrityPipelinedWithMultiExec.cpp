...
// Global counter key that will be used for all INCR requests.
 ...
 *   (2) Send MULTI.
 *   (3) Inside MULTI: send only one INCR command on the global counter.
 *   (4) Send EXEC.
 *   (5) Read and discard replies.
 *   (6) Save the normal keys (and their expected values) in globalData for later verification.
 */
                          ...
                          int pipelineDepth, int iterations, int threadId)
{
    ...
    for (int iter = 0; iter < iterations; iter++) {
        ...
        // (1) Outside MULTI: send SET commands.
        ...
        // (3) Inside MULTI: send only one INCR command on the global counter.
        redisAppendCommand(conn, "INCR %s", globalCounterKey.c_str());

        // (4) Send EXEC.
        redisAppendCommand(conn, "EXEC");

        // (5) Read replies.
        // a) Replies for the outside SET commands.
        for (int i = 0; i < pipelineDepth; i++) {
        ...
        // b) Reply for MULTI.
        {
            redisReply* r = nullptr;
            if (redisGetReply(conn, (void**)&r) != REDIS_OK)
                std::cerr << "Error reading reply for MULTI\n";
            if (r) freeReplyObject(r);
        }
        // c) One queued reply for the INCR command inside MULTI.
        {
            redisReply* r = nullptr;
            if (redisGetReply(conn, (void**)&r) != REDIS_OK)
                std::cerr << "Error reading QUEUED reply inside MULTI\n";
            if (r) freeReplyObject(r);
        }
        // d) EXEC reply.
        {
            redisReply* execReply = nullptr;
            if (redisGetReply(conn, (void**)&execReply) != REDIS_OK)
                std::cerr << "Error reading EXEC reply\n";
            if (execReply) freeReplyObject(execReply);
        }

        // (6) Save the normal key and its expected value for later verification.
        {
                              ...
                              int pipelineDepth, int iterations)
{
    // Write phase.
    {
        ...
        for (int iter = 0; iter < iterations; iter++) {
            ...
            redisAppendCommand(conn, "MULTI");
            // Only one INCR command inside MULTI.
            redisAppendCommand(conn, "INCR %s", globalCounterKey.c_str());
            redisAppendCommand(conn, "EXEC");
            for (int i = 0; i < pipelineDepth; i++) {
                redisReply* r = nullptr;
            ...
            }
            {
                redisReply* r = nullptr;
                if (redisGetReply(conn, (void**)&r) != REDIS_OK)
                    std::cerr << "Error reading MULTI reply in single-conn write\n";
                if (r) freeReplyObject(r);
            }
            // Only one QUEUED reply for the single INCR command.
            {
                redisReply* r = nullptr;
                if (redisGetReply(conn, (void**)&r) != REDIS_OK)
                    std::cerr << "Error reading QUEUED reply in single-conn write\n";
                if (r) freeReplyObject(r);
            }
            {
                redisReply* execReply = nullptr;
                if (redisGetReply(conn, (void**)&execReply) != REDIS_OK)
                    std::cerr << "Error reading EXEC reply in single-conn write\n";
                if (execReply) freeReplyObject(execReply);
            }
            {
                std::lock_guard<std::mutex> lock(globalDataMutex);
...
int main(int argc, char* argv[]) {
    ...
    // Final verification of the global counter key.
    // Expected count = number of writer INCR requests:
    // For single connection: iterations,
    // For multi-connection: numWriters * iterations.
    long long expectedCount = 0;
    if (numConnections == 1)
        expectedCount = 1LL * iterations;
    else {
        int numWriters = (numConnections + 1) / 2;
        expectedCount = 1LL * numWriters * iterations;
    }
    redisContext* conn = connectRedis(host, port);
    redisReply* reply = (redisReply*)redisCommand(conn, "GET %s", globalCounterKey.c_str());
    if (reply && reply->type == REDIS_REPLY_STRING && reply->str) {
        long long counterValue = 0;
        try {
            counterValue = std::stoll(reply->str);
        } catch (...) {
            counterValue = -1;
        }
        if (counterValue != expectedCount) {
            std::cerr << "ERROR: Global counter value (" << counterValue
                      << ") does not match expected count (" << expectedCount << ")\n";
        } else {
            std::cout << "Global counter value (" << counterValue
                      << ") matches expected count (" << expectedCount << ")\n";
        }
    } else {
        std::cerr << "ERROR: Could not fetch global counter value.\n";
    }
